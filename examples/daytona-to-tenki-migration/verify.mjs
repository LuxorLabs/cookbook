/**
 * Proves the Tenki side of the Daytona->Tenki migration end to end:
...
let sandbox;
try {
	// Daytona: await daytona.create()  ->  Tenki: createAndWait() boots + waits for ready.
	...
	// Daytona: sandbox.process.executeCommand("...") runs via a shell. Tenki: exec() runs a bare
	// binary + args, so port a shell command by wrapping it in `sh -c`.
	const shell = await sandbox.exec("sh", { args: ["-c", "echo hello-from-tenki"] });
	// Daytona: result.result is a string. Tenki: bytes -> decode with stdoutText().
	...
	// Daytona: sandbox.process.codeRun("print(6 * 7)") runs a code snippet. Tenki: run the
	// interpreter directly as a bare binary + args (no shell needed).
	...
	console.log(`✓ daytona-to-tenki-migration: create (${sandbox.id.slice(0, 8)}…) → exec sh + python3 → "${out}" / 42 → dispose`);
} catch (e) {
	console.error("✗ " + (e?.message ?? e));
	process.exitCode = 1;
} finally {
	if (sandbox) {
		// Daytona: await daytona.delete(sandbox)  ->  Tenki: close() terminates the sandbox.
		try {
			await sandbox.close();
		} catch {
